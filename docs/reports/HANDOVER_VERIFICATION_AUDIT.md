...
| M10 | **NOT DONE** | Active Q&A still bypasses hybrid retrieval (`apps/qa/main.py:383-388` passes `pool=None, embedder=None, _override_chunks`); `source_id` is the patient id on every chunk (`main.py:135,170,192,217,250,280,302`); ~~`_BM25_SQL` still has no `ORDER BY`~~ -- **corrected 2026-09-18**: both SQL arms do order their rows (`packages/retrieval/src/retrieval/retriever.py:42` and `:60-65`), and that is now pinned by `packages/retrieval/tests/test_sql_ordering.py`. The line numbers in this row belong to an older revision of the file: `retriever.py:30-47` is `_VECTOR_SQL` today. The other two clauses stand. Commit `c8b16a0` claims typed fact contracts but the diff contains none. |
