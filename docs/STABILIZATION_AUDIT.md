...
- [ ] **M-2a** Remove the optimistic success state from `ReceptionistTab`'s
...
- [ ] **L-4** Add an `icd10Code` to mock order `o5`.

### Phase 2 — Make the demo honest (highest visible payoff)

- [ ] **H-4** Add a realistic `evidenceChain` to at least one mock order line
...
- [ ] **H-3a** Replace `AmbientScribePane`'s mock recording with the existing
