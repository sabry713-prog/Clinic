...
### Arabic + code-switching expansion (June 2026)

Brought AR rules-only sensitivity 0.25 → 1.00 and code-switching 0.53 → 0.95, both with zero false refusals (specificity 1.00). Patterns are deliberately narrow to avoid refusing factual Arabic lookups (e.g. bare جرعة "dose" / تحليل "test" are **not** matched — only "increase dose" / "repeat test"):

- **TREND**: verb forms يتدهور/تتدهور/يتحسن/تتحسن/تسوء/يسوء (bare nouns تدهور/تحسن excluded — they occur in risk phrasing); code-switch "showing signs of تحسن/تدهور".
- **DIAGNOSTIC**: يسبب/تشير إلى/يدل على/على الأرجح; "هل هذا <acute-dx>" (enumerated disease nouns); code-switch "what is going on", "is it (an) infection?", "هل هذا sepsis".
- **TREATMENT**: الدواء/العلاج/الجرعة المناسبة, زيادة/تعديل/تخفيض الجرعة, يجب وصف/وصفه.
- **MEDICATION_SAFETY**: آمن/آمنة, تفاعل/تفاعلات, موانع الاستعمال, مضاد استطباب.
- **RISK**: (ما) خطورة وضع, توقعات الشفاء/التعافي, المآل, الإنذار.
