...
// Dev pharmacist user -- linked to Keycloak dev realm user "pharmacist1".
// Needed to exercise the refill-queue endpoints (refill_request:fulfill)
// locally without a real pharmacist account. Note: this id was generated by
// Keycloak on `kcadm create` (the admin REST API always server-generates
// user IDs, unlike realm-import which honors an explicit id) — it matches
// infra/keycloak/realm-dev.json's pharmacist1 id, which a FRESH import will
// assign verbatim (import, unlike the admin API, does respect explicit ids).
const DEV_PHARMACIST_EXTERNAL_SUBJECT = "8836c614-61d7-4a91-9f7b-a51955162aa8";
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    const pharmacistResult = await client.query<{ id: string }>(
      `INSERT INTO app."user"
         (tenant_id, external_subject, email, display_name, preferred_language)
       VALUES ($1, $2, $3, $4, 'ar')
       ON CONFLICT (tenant_id, external_subject) DO UPDATE
         SET display_name = EXCLUDED.display_name
       RETURNING id`,
      [TENANT_ID, DEV_PHARMACIST_EXTERNAL_SUBJECT, "dev-pharmacist@dev.hospital.local", DEV_PHARMACIST_DISPLAY_NAME],
    );

    const pharmacistDbId = pharmacistResult.rows[0]?.id;
    if (!pharmacistDbId) throw new Error("Failed to insert dev pharmacist");
    ...
    console.log(`Dev pharmacist ID: ${pharmacistDbId}`);
    ...
    console.log(`Dev pharmacist external_subject: ${DEV_PHARMACIST_EXTERNAL_SUBJECT}`);
