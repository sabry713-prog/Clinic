...
async def health() -> dict[str, Any]:
    """Liveness plus the two things this service cannot generate without.

    H02: this returned {"status": "ok"} unconditionally. The model client is built
    here (no network call -- a probe that spends a completion on every health check
    would be its own outage), and the template versions are reported because a
    narrative generated by an unrecognised template version is not reviewable.
    """
    try:
        get_model()
        model = "configured"
    ...
    return {
        "status": "ok" if model == "configured" else "degraded",
        "service": settings.otel_service_name,
        "model": model,
        "templates": {
            "interpreter": INTERPRETER_TEMPLATE_VERSION,
            "patient_recap": PATIENT_RECAP_TEMPLATE_VERSION,
        },
    }
