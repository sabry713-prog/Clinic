...
function CategoryIcon({ category }: { readonly category: string }): JSX.Element {
  ...
  return (
    <div className="shrink-0 w-9 h-9 rounded-lg bg-veil border border-line flex items-center justify-center">
      <svg className="w-5 h-5 text-ink-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
...
function ServiceCandidateCard({
  ...
  const c = candidate;
  return (
    <li className="border border-line bg-white shadow-card rounded-xl px-4 py-3">
      <div className="flex items-center gap-3">
        <CategoryIcon category={c.category} />
        <div className="min-w-0 flex-1">
          <p className="text-sm text-ink truncate" dir="ltr">
            <span className="text-ink-soft">Create</span>{" "}
            <span className="font-medium bg-ev-pill-bg border border-ev-pill-line text-ev-pill rounded-md px-1.5 py-0.5">{c.code_display}</span>{" "}
            <span className="text-ink-soft">order</span>{" "}
            <span className="text-xs text-ink-soft">({c.category})</span>
          </p>
          <button type="button" onClick={onToggleExpanded} className="text-xs text-ink-soft hover:text-ink-deep mt-0.5">
      ...
      {isExpanded && (
        <p className="mt-2 ms-12 text-xs text-ink-soft border-s-2 border-line ps-3" dir="auto">
  ...
  return (
    <div className="bg-white border border-line rounded-lg p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-ink">Service Requests</h2>
        <span className="text-xs text-ink-soft">From the doctor's documented orders — confirm before creating</span>
      </div>
      ...
      {/* Existing requests */}
      <div>
        <h3 className="text-sm font-medium text-ink-deep mb-2">Created requests</h3>
        {existing.length === 0 ? (
          <p className="text-sm text-ink-soft">None yet</p>
        ) : (
          <ul className="space-y-1.5">
            {existing.map((s) => (
              <li key={s.id} className="text-sm text-ink flex items-center gap-2 flex-wrap" dir="ltr">
                <span className="inline-block rounded bg-veil text-ink-deep text-xs px-1.5 py-0.5">{s.category}</span>
                {s.code_display}
                <span className="text-ink-soft">({s.status}, {formatDate(s.requested_at)})</span>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-sm px-3 py-1.5 rounded bg-grad-accent hover:brightness-110 shadow-pill text-white disabled:opacity-50"
            >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-ink-deep">Quick order entry</h3>
          <select
            value={quickLanguage}
            disabled={quickBusyOrRecording}
            onChange={(e) => setQuickLanguage(e.target.value as "en" | "ar")}
            className="bg-veil text-ink-deep text-xs border border-line-strong rounded px-2 py-1"
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            dir="auto"
            className="flex-1 bg-veil border border-line-strong rounded px-2.5 py-1.5 text-sm text-ink-deep disabled:opacity-50"
          />
          <button
            type="button"
            ...
            className={`text-xs px-2.5 py-1.5 rounded-lg text-ink disabled:opacity-50 ${
              quickDictation.recording ? "bg-red-600 hover:bg-red-500 animate-pulse" : "bg-white hover:bg-veil border border-line"
            }`}
        ...
        {(quickError ?? quickDictation.error) && (
          <p className="text-sm text-ink-soft">{quickError ?? quickDictation.error}</p>
        )}
