  ...
  return (
    <div className="bg-white border border-line rounded-2xl shadow-card p-4 space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        <h2 className="text-sm font-medium text-ink-deep">NPHIES rejection analytics</h2>
        <div>
          <label htmlFor="rej-since" className="block text-xs text-ink-soft mb-1">Since</label>
        ...
        <div>
          <label htmlFor="rej-until" className="block text-xs text-ink-soft mb-1">Until</label>
        ...
        <div className="space-y-5 text-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={String(label)} className="bg-veil rounded p-3">
          ...
          <div>
            <h3 className="font-medium mb-2 text-ink">Claims per week (rejected shown against total)</h3>
            {data.by_week.length === 0 ? (
              <p className="text-ink-soft">No claims in range.</p>
            ) : (
              <div className="flex items-end gap-1 h-32">
                {data.by_week.map((w) => (
                  <div key={w.week} className="flex-1 flex flex-col items-center justify-end gap-1" title={`${formatWeek(w.week)}: ${w.total} total, ${w.rejected} rejected`}>
                    <div className="w-full rounded-t bg-veil relative" style={{ height: `${Math.max(4, (w.total / maxWeekTotal) * 100)}px` }}>
                      {w.rejected > 0 && (
                        <div
                          className="absolute bottom-0 left-0 right-0 rounded-t bg-ink-faint"
                          style={{ height: `${(w.rejected / w.total) * 100}%` }}
                        />
                      )}
                    </div>
                    <span className="text-[10px] text-ink-soft">{formatWeek(w.week)}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <h4 className="font-medium mb-1 text-ink">Claims by status</h4>
            ...
            <div>
              <h4 className="font-medium mb-1 text-ink">Top rejection codes</h4>
              {data.by_rejection_code.length === 0 ? (
                <p className="text-ink-soft">No rejections in range.</p>
              ) : (
                <table className="w-full">
                  <tbody>
                    {data.by_rejection_code.map((r) => (
                      <tr key={r.code} className="border-b border-line">
                        <td className="py-1 pr-2 text-ink-deep font-mono text-xs">{r.code}</td>
                        <td className="py-1 text-right text-ink">{r.count}</td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <p className="text-xs text-ink-soft">{data.disclaimer}</p>
        </div>
      )}
    </div>
  );
