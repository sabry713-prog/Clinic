...
import { resolveCurrentView } from "./viewRouting";
...
const ICONS: Record<string, string> = {
  ...
  interpreter: "M10.5 21l5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 016-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 01-3.827-5.802",
...
export default function AppShell(): JSX.Element {
  ...
  const currentView = resolveCurrentView(onPatientPage, viewParam);
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3 space-y-1">
          ...
          {activePatientId && (
            <>
              ...
              />
              {/* Occasional tools: not part of the per-encounter flow, so they live
                  here rather than competing with the journey's stages. Each opens its
                  panel on the patient page (?open=), which is where the panel renders. */}
              {(["narrative", "handoff", "refills", "interpreter", "search"] as const).map((id) => (
                <NavItem
                  key={id}
                  icon={id}
                  label={t(`shell.tools.${id}`)}
                  active={openCardParam === id}
                  collapsed={collapsed}
                  onClick={() => goToCard(id)}
                />
              ))}
          ...
          {isAdmin && (
            <>
              ...
              <NavItem
                icon="claimCheck"
                label={t("shell.claimSimulator")}
                active={location.pathname === "/admin/claim-simulator"}
                collapsed={collapsed}
                onClick={() => void navigate("/admin/claim-simulator")}
              />
              <NavItem
                icon="checkBadge"
                label={t("shell.coderQueue")}
                active={location.pathname === "/admin/coder-queue"}
                collapsed={collapsed}
                onClick={() => void navigate("/admin/coder-queue")}
              />
            </>
          )}
        </nav>
