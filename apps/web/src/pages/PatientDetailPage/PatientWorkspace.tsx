...
interface ChipDef {
  ...
  /** Agent identity hue (mockup §F) — one color per mode everywhere. */
  readonly hue: string;
}

const CHIPS: readonly ChipDef[] = [
  { id: "qa", label: "Consultant", icon: "M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z", hue: "text-agent-cons" },
  { id: "diagnosis", label: "Diagnosis", icon: "M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z", hue: "text-violet-500" },
  { id: "narrative", label: "Researcher", icon: "M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z", hue: "text-sky-500" },
  { id: "handoff", label: "Handoff", icon: "M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5", hue: "text-teal-500" },
  { id: "draft", label: "Draft", icon: "M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10", hue: "text-purple-500" },
  { id: "orders", label: "Orders", icon: "M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z", hue: "text-slate-500" },
  { id: "claims", label: "Coder", icon: "M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25z", hue: "text-agent-nph" },
  { id: "search", label: "Search", icon: "M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z", hue: "text-slate-400" },
  { id: "interpreter", label: "Interpreter", icon: "M10.5 21l5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 016-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 01-3.827-5.802", hue: "text-amber-500" },
  { id: "ambient", label: "Scribe", icon: "M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z", hue: "text-agent-scribe" },
  { id: "refills", label: "Refills", icon: "M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99", hue: "text-agent-pharm" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-baseline gap-3 text-sm">
        <span className="font-medium text-ink" dir="ltr">{patient.display_name ?? patient.mrn}</span>
        <span className="text-ink-soft" dir="ltr">{patient.mrn}</span>
        {patient.allergies.length > 0 && (
          <span className="text-ink-soft truncate" dir="ltr">
      ...
      {/* Composer */}
      <div className="bg-white border border-line rounded-[18px] shadow-card p-3 space-y-3">
        <form onSubmit={submitComposer} className="flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-transparent text-sm text-ink placeholder-ink-faint focus:outline-none px-2 py-1.5"
          />
          <button
            type="submit"
            disabled={!composerText.trim()}
            className="shrink-0 text-xs px-4 py-1.5 rounded-full bg-grad-accent hover:brightness-110 text-white font-semibold shadow-pill disabled:opacity-40 disabled:shadow-none transition-all"
          >
            Ask
          </button>
        </form>
      ...
      {openCards.length === 0 ? (
        ...
        <div className="space-y-4">
          {openCards.map((id) => (
            <div key={id} id={`workspace-card-${id}`} className="bg-white border border-line rounded-2xl shadow-card overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-line">
                <span className="text-sm font-medium text-ink">{CARD_LABEL[id]}</span>
                <button
                  type="button"
