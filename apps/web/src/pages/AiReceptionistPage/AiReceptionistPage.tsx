...
export default function AiReceptionistPage(): JSX.Element {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F4F7FE] via-[#F6F4FE] to-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-agent-nph mt-1 text-sm">No account needed -- verify your phone to get started</p>
        </div>

        <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-8 shadow-2xl space-y-4">
          {error && (
            <div role="alert" className="p-3 rounded-lg bg-red-500/20 border border-red-500/30 text-status-rej text-sm text-center">
              {error}
            </div>
          )}
            ...
            <>
              <label className="block text-sm text-ink-deep">
                ...
                <input
                  ...
                  dir="ltr"
                  className="mt-1 w-full bg-white border border-line rounded-lg px-3 py-2 text-ink"
                />
              </label>
              <button
                type="button"
                ...
                className="w-full py-3 rounded-xl bg-grad-accent hover:brightness-110 shadow-pill disabled:opacity-50 text-white font-medium text-sm"
              >
          ...
          {step === "otp" && (
            <>
              <p className="text-sm text-ink-deep text-center">Enter the 6-digit code sent to {phone}</p>
              <input
                type="text"
                value={code}
                ...
                maxLength={6}
                dir="ltr"
                className="w-full bg-white border border-line rounded-lg px-3 py-2 text-ink text-center tracking-[0.5em] text-lg"
              />
              <button
                type="button"
          ...
          {step === "intent" && (
            <>
              <p className="text-sm text-ink-deep">What would you like to book?</p>
              <textarea
                value={intentText}
                onChange={(e) => setIntentText(e.target.value)}
                rows={2}
                placeholder="e.g. follow-up with Cardiology"
                className="w-full bg-white border border-line rounded-lg px-3 py-2 text-ink resize-none"
              />
              <button
                type="button"
                onClick={() => void searchIntent()}
                disabled={busy || !intentText.trim()}
                className="w-full py-2.5 rounded-xl border border-line-strong text-ink-deep hover:border-line-strong text-sm"
              >
                {busy ? "Searching…" : "Search"}
              </button>

              {matchConfirming ? (
                <div className="p-3 rounded-xl bg-agent-cons-bg border border-ev-pill-line text-sm text-ink-deep text-center space-y-2">
                  <p>
                    {departmentDisplay}
                    {appointmentType && ` · ${APPOINTMENT_TYPES.find((t) => t.value === appointmentType)?.label ?? appointmentType}`}
                  </p>
                  <div className="flex gap-2">
                    <button type="button" onClick={() => void loadSlots()} className="flex-1 py-2 rounded-lg bg-grad-accent hover:brightness-110 shadow-pill text-white text-xs">
                      That's right
                    </button>
                    <button type="button" onClick={() => setMatchConfirming(false)} className="flex-1 py-2 rounded-lg border border-line-strong text-ink-deep text-xs">
                      Pick manually
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-white/10">
                  <label className="block text-xs text-ink-soft">
                    ...
                    <select
                      ...
                      className="mt-1 w-full bg-white border border-line rounded-lg px-3 py-2 text-ink text-sm"
                    >
              ...
              <button type="button" onClick={() => void openMyAppointments()} className="w-full text-xs text-ink-soft hover:text-ink-deep underline">
          ...
          {step === "slots" && (
            <>
              <p className="text-sm text-ink-deep text-center">
                {departmentDisplay} — pick a time in the next {LOOKAHEAD_DAYS} days
              </p>
              {slots.length === 0 ? (
                <p className="text-sm text-ink-soft text-center">No open slots in this window</p>
              ) : (
                <div className="max-h-72 overflow-y-auto space-y-1.5">
                  {slots.map((s) => (
                    <button
                      ...
                      disabled={busy}
                      className="w-full text-left px-3 py-2 rounded-lg border border-line hover:border-blue-400 text-ink-deep text-sm disabled:opacity-50"
                    >
              ...
              <button type="button" onClick={() => setStep("intent")} className="w-full text-xs text-ink-soft hover:text-ink-deep underline">
                Back
              </button>
            </>
          )}
          ...
          {step === "success" && bookedAppointment && (
            <div className="text-center space-y-3">
              ...
              <p className="text-ink-soft text-xs">{bookedAppointment.department_display}</p>
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => { setStep("intent"); setIntentText(""); setMatchConfirming(false); setDepartmentDisplay(""); setAppointmentType(""); }}
                  className="flex-1 py-2.5 rounded-xl border border-line-strong text-ink-deep text-sm"
                >
                  Book another
                </button>
                <button type="button" onClick={() => void openMyAppointments()} className="flex-1 py-2.5 rounded-xl bg-grad-accent hover:brightness-110 shadow-pill text-white text-sm">
                  My appointments
                </button>
              </div>
            </div>
          ...
          {step === "my-appointments" && (
            <>
              <p className="text-sm text-ink-deep text-center">Your appointments</p>
              {myAppointments === null || myAppointments.length === 0 ? (
                <p className="text-sm text-ink-soft text-center">No appointments yet</p>
              ) : (
                <ul className="space-y-2">
                  {myAppointments.map((a) => (
                    <li key={a.id} className="p-3 rounded-lg border border-line flex items-center justify-between gap-2">
                      <div>
                        <p className="text-sm text-ink">{a.appointment_type} — {formatSlot(a.scheduled_at, i18n.language)}</p>
                        <p className="text-xs text-ink-soft">{a.department_display} · {a.status}</p>
                      </div>
                      {a.status === "scheduled" && (
                        <button
                          type="button"
                          ...
                          disabled={busy}
                          className="text-xs text-ink-soft hover:text-ink-deep underline disabled:opacity-50 shrink-0"
                        >
                          Cancel
                        </button>
                      )}
